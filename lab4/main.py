...
iteration_count = input('Enter the number of iteration(min:10 max:1000): ')
while not iteration_count.isdigit() or (int(iteration_count) > 1000 or int(iteration_count) < 10):
    iteration_count = input('Enter the number of iteration(min:10 max:1000): ')
...
start_time = time()
ant_colony = AntColony(distances, int(ant_count), int(iteration_count), 0.6, alpha=2, beta=4)
